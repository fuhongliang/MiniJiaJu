...
Page({
        ...
        o.request({
            ...
            success: function(t) {
              console.log(t.data);
                if (0 == t.code) {
                  var list = [];
                  for(var iii in t.data.coupon_list){
                    if(parseInt(t.data.coupon_list[iii].is_receive) === 1){
                      list.push(parseInt(iii));
                      console.log(list);
                      console.log(parseInt(t.data.coupon_list[iii].is_receive));
                      a.setData({
                        isGet:list
                      })
                    }
  ...
  onConfirm: function(t) {
    ...
    for (
      var c = a.data.carGoods, _ = JSON.parse(e.attr), l = _.length, s = 0;
      s < l;
      s++
    ) {
      var u = _[s].attr_list;
    ...
    if (h <= 0) (w = 1), c.push(p);
      ...
      for (N = 0; N < h; N++)
        c[N].goods_id == e.id &&
          JSON.stringify(c[N].attr) == JSON.stringify(r) &&
          ((c[N].num += 1),
          (w = c[N].num),
          (c[N].goods_price = w * f),
          (c[N].goods_price = c[N].goods_price.toFixed(2)),
          (v = !1));
      v && (c.push(p), (w = 1));
    }
    if (w > m) {
      ...
      var k = a.data.total;
      (k.total_num += 1),
        (k.total_price = parseFloat(k.total_price)),
        (f = parseFloat(f)),
        (k.total_price += f),
        (k.total_price = k.total_price.toFixed(2));
      ...
      if (q) {
        for (var b = q.length, y = [], s = 0; s < b; s++)
          for (var F = q[s].goods, G = F.length, N = 0; N < G; N++)
            y.push(F[N]);
        for (var O = y.length, M = [], T = 0; T < O; T++)
          y[T].id == a.data.id && M.push(y[T]);
  ...
  guigejian: function(t) {
    ...
    for (
      var n = a.data.goods, c = JSON.parse(n.attr), _ = c.length, e = 0;
      e < _;
      e++
    ) {
      var l = c[e].attr_list;
    ...
    if (0 == u) g = parseFloat(n.price);
    else var g = parseFloat(u);
    for (var m = a.data.carGoods, f = m.length, p = 0; p < f; p++)
      if (
        m[p].goods_id == n.id &&
        JSON.stringify(m[p].attr) == JSON.stringify(r)
      ) {
        m[p].num -= 1;
        var h = m[p].num;
        m[p].goods_price = h * g;
    ...
    var w = a.data.total;
    (w.total_num -= 1),
      (w.total_price = parseFloat(w.total_price)),
      (u = parseFloat(u)),
      (w.total_price -= u),
      (w.total_price = w.total_price.toFixed(2));
    ...
    if (D) {
      for (var S = D.length, q = [], e = 0; e < S; e++)
        for (var b = D[e].goods, y = b.length, p = 0; p < y; p++) q.push(b[p]);
      for (var F = q.length, G = [], N = 0; N < F; N++)
        q[N].id == a.data.id && G.push(q[N]);
  ...
  jia: function(t) {
    var a = this,
      o = a.data.goods,
      i = a.data.goods_num;
    if ((i += 1) > JSON.parse(o.attr)[0].num)
      return (
        wx.showToast({
          title: "商品库存不足",
          image: "/images/icon-warning.png"
    ...
    (r.total_num += 1),
      ...
      (r.total_price = parseFloat(r.total_price)),
      (r.total_price += o.price),
    ...
    var e = parseFloat(o.price * i),
      ...
      d = a.data.carGoods,
      n = !0;
    if ((u = d.length) <= 0) d.push(s);
    else {
      for (h = 0; h < u; h++)
        d[h].goods_id == o.id &&
          ((d[h].num += 1), (d[h].goods_price = e.toFixed(2)), (n = !1));
      n && d.push(s);
    }
    ...
    if (l) {
      for (var u = l.length, g = [], m = 0; m < u; m++)
        for (var f = l[m].goods, p = f.length, h = 0; h < p; h++) g.push(f[h]);
      for (var v = g.length, w = [], x = 0; x < v; x++)
        g[x].id == a.data.id && w.push(g[x]);
  ...
  jian: function() {
    ...
    (i.total_num -= 1),
      ...
      (i.total_price = parseFloat(i.total_price)),
      (i.total_price -= a.price),
      (i.total_price = i.total_price.toFixed(2));
    for (var r = t.data.carGoods, e = r.length, s = 0; s < e; s++)
      r[s].goods_id == a.id &&
        ((r[s].num -= 1),
        (r[s].goods_price = parseFloat(r[s].goods_price)),
        (r[s].goods_price -= a.price),
        (r[s].goods_price = r[s].goods_price.toFixed(2)));
    ...
    if (c) {
      for (var e = c.length, _ = [], l = 0; l < e; l++)
        for (var u = c[l].goods, g = u.length, s = 0; s < g; s++) _.push(u[s]);
      for (var m = _.length, f = [], p = 0; p < m; p++)
        _[p].id == t.data.id && f.push(_[p]);
  ...
  tianjia: function(t) {
    ...
    if (i) {
      ...
      if (1 == m.length) {
        ...
        (f.goods_price = parseFloat(f.goods_price)),
          (o.price = parseFloat(o.price)),
          (f.goods_price += o.price),
          (f.goods_price = f.goods_price.toFixed(2));
        for (var p = u.length, h = 0; h < p; h++) u[h].num += 1;
      } else {
        for (var p = u.length, h = 0; h < p; h++) u[h].num += 1;
        for (var v = (F = a.data.carGoods).length, w = [], _ = 0; _ < v; _++)
          if (o.index == _)
            for (var x = F[_].attr, k = x.length, d = 0; d < k; d++) {
              var D = {
                attr_id: x[d].attr_id,
        ...
        for (var S = m.length, d = 0; d < S; d++)
          if (JSON.stringify(m[d].attr_list) == JSON.stringify(w))
            var q = m[d].num;
        for (_ = 0; _ < v; _++)
          if (
            o.index == _ &&
            ((F[_].num += 1),
            (F[_].goods_price = parseFloat(F[_].goods_price)),
            (o.price = parseFloat(o.price)),
            (F[_].goods_price += o.price),
            (F[_].goods_price = F[_].goods_price.toFixed(2)),
            F[_].num > q)
          ) {
            wx.showToast({
              title: "商品库存不足",
            ...
            return (
              (F[_].goods_price -= o.price),
              void (F[_].goods_price = F[_].goods_price.toFixed(2))
            );
          }
      }
      ...
      var F = a.data.carGoods,
        G = F[o.index].attr,
        N = [];
      if (G)
        for (var O = G.length, _ = 0; _ < O; _++)
          N.push({
            attr_id: G[_].attr_id,
      ...
      if (
        ((F[o.index].num += 1),
        (F[o.index].goods_price = parseFloat(F[o.index].goods_price)),
        (o.price = parseFloat(o.price)),
        (F[o.index].goods_price += o.price),
        (F[o.index].goods_price = F[o.index].goods_price.toFixed(2)),
        F[o.index].num > J)
      )
        return (
          wx.showToast({
  ...
  jianshao: function(t) {
    for (
      ...
      if (o.index == e) {
        ...
        (i[e].num -= 1),
          (o.price = parseFloat(o.price)),
          (i[e].goods_price = parseFloat(i[e].goods_price)),
          (i[e].goods_price -= o.price),
          (i[e].goods_price = i[e].goods_price.toFixed(2));
      }
    a.setData({
      carGoods: i
    });
    ...
    if (s) {
      ...
      for (var l = [], u = d.length, e = 0; e < u; e++)
        o.id == d[e].id && l.push(d[e]);
      for (var g = l.length, m = 0; m < g; m++)
        l[m].id == o.id && (l[m].num -= 1);
    ...
    var h = a.data.total;
    (h.total_num -= 1),
      (h.total_price = parseFloat(h.total_price)),
      (h.total_price -= o.price),
  ...
  clearCar: function() {
    ...
    t.data.goodsModel;
    for (var o = t.data.carGoods, i = o.length, r = 0; r < i; r++)
      (o[r].num = 0), (o[r].goods_price = 0);
    for (var e = t.data.quick_list, i = e.length, r = 0; r < i; r++)
      for (var s = e[r].goods, d = s.length, n = 0; n < d; n++) s[n].num = 0;
    for (var c = t.data.quick_hot_goods_lists, _ = c.length, l = 0; l < _; l++)
      c[l].num = 0;
    t.setData({
      carGoods: o,
      total: a,
  ...
  buynow: function(t) {
    ...
    for (var i = o.length, r = [], e = [], s = 0; s < i; s++)
      0 != o[s].num &&
        ((e = {
          id: o[s].goods_id,
          num: o[s].num,
        ...
        console.log(r);
    wx.navigateTo({
      url: "/order/order-submit/order-submit?cart_list=" + JSON.stringify(r)
    });
  },
  selectDefaultAttr: function() {
    ...
    if (t.data.goods && 0 === t.data.goods.use_attr) {
      for (var a in t.data.attr_group_list)
        for (var o in t.data.attr_group_list[a].attr_list)
          0 == a &&
            0 == o &&
            (t.data.attr_group_list[a].attr_list[o].checked = !0);
      t.setData({
        attr_group_list: t.data.attr_group_list
      });
    }
          ...
          success: function(t) {
            ...
            0 == t.code &&
              ...
              i.setData({
                ...
                comment_list: a
                  ? i.data.comment_list.concat(t.data.list)
                  : t.data.list
              }),
  ...
  onGoodsImageClick: function(t) {
    ...
    for (var r in a.data.goods.pic_list)
      o.push(a.data.goods.pic_list[r].pic_url);
    wx.previewImage({
      urls: o,
      current: o[i]
    });
  ...
  submit: function(a) {
    ...
    if (
      i.data.miaosha_data &&
      i.data.miaosha_data.rest_num > 0 &&
      i.data.form.number > i.data.miaosha_data.rest_num
    )
      return (
        wx.showToast({
    ...
    if (i.data.form.number > i.data.goods.num)
      return (
        wx.showToast({
          title: "商品库存不足，请选择其它规格或数量",
          image: "/images/icon-warning.png"
    ...
    "ADD_CART" == a &&
      ...
      "BUY_NOW" == a &&
        ...
        wx.redirectTo({
          url:
            "/order/order-submit/order-submit?goods_info=" +
            JSON.stringify({
              goods_id: i.data.id,
              attr: e,
              num: i.data.form.number
        ...
        console.log(i.data.id,e,i.data.form.number);
  },
  ...
  gotoGet: function(tt) {
    ...
    o.request({
      ...
      data:{
        id:tt.currentTarget.dataset.eid
      },
      method:"get",
      success:function(res){
        ...
        if(res.code == 0){
          ...
          console.log(isGet);
          let id = tt.currentTarget.dataset.id;
  ...
  attrClick: function(a) {
    ...
    for (var d in s)
      if (s[d].attr_group_id == r)
        for (var n in s[d].attr_list)
          s[d].attr_list[n].attr_id == e
            ? (s[d].attr_list[n].checked = !0)
            : (s[d].attr_list[n].checked = !1);
    i.setData({
      attr_group_list: s
    });
    var c = [],
    ...
    if (u) {
      for (var g = u.length, m = [], d = 0; d < g; d++) {
        ...
        if (f) {
          for (var p = f.length, m = [], h = 0; h < p; h++)
            m.push(f[h].attr_id);
          v = 0;
          if (c.join(",") == m.join(",")) {
            v = u[d].num;
    ...
    _ &&
      o.request({
        ...
        success: function(t) {
          if (0 == t.code) {
            var a = i.data.goods;
            (a.price = t.data.price),
              (a.num = t.data.num),
              (a.attr_pic = t.data.pic),
              i.setData({
                goods: a,
  ...
  tabSwitch: function(t) {
    var a = this;
    "detail" == t.currentTarget.dataset.tab
      ? a.setData({
          tab_detail: "active",
          tab_comment: ""
        })
  ...
  onReachBottom: function() {
    var t = this;
    "active" == t.data.tab_detail && t.data.drop
      ? ((t.data.drop = !1),
        t.goods_recommend({
