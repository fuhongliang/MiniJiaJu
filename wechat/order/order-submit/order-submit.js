...
Page({
  data: {
    ...
    show_payment: !1,
    other_mobile: "",
    store_logo: wx.getStorageSync("store").store_logo,
    show_evaluate: false,
    isGet: [],
  ...
  bindkeyinput: function(t) {
    ...
    -1 == a
      ...
      : (this.data.mch_list[a] &&
          (this.data.mch_list[a].content = t.detail.value),
        this.setData({
  ...
  getOffline: function(t) {
    ...
    if (1 == t.currentTarget.dataset.index)
      ...
      var s = a.data.is_area_city_id,
        o = a.data.address ? a.data.address.city_id : null;
      o &&
        s &&
        (function(t, a) {
          ...
          success: function(a) {
            a &&
              (a.authSetting["scope.userLocation"]
                ...
                : wx.showToast({
                    title: "您取消了授权",
                    image: t.data.img_url + "/images/icon-warning.png"
                  }));
          }
        });
      }
  ...
  orderSubmit: function(t) {
    ...
    if (0 == e) {
      ...
      if (
        ...
        (function(t, a) {
          for (var e = 0; e < t.length; e++) if (a == t[e]) return !1;
          return !0;
        })(s, a.data.address.city_id)
      )
        return void wx.showToast({
          title: "所选地区无货",
          image: a.data.img_url + "/images/icon-warning.png"
        });
    }
    if (0 == e) {
      if (!a.data.address || !a.data.address.id)
        return void wx.showToast({
          title: "请选择收货地址",
          image: a.data.img_url + "/images/icon-warning.png"
        });
      ...
      if (
        ((i.shop_id = a.data.shop.id),
        !i.address_name || void 0 == i.address_name)
      )
        return void a.showToast({
          title: "请填写收货人",
          image: a.data.img_url + "/images/icon-warning.png"
        });
      if (!i.address_mobile || void 0 == i.address_mobile)
        return void a.showToast({
          title: "请填写联系方式",
          image: a.data.img_url + "/images/icon-warning.png"
        });
      if (!/^\+?\d[\d -]{8,12}\d/.test(i.address_mobile))
        return void wx.showModal({
          title: "提示",
          content: "手机号格式不正确",
          showCancel: !1
    ...
    if (1 == o.is_form && a.data.goods_list && a.data.goods_list.length) {
      ...
      for (var d in n)
        if (
          ("date" == n[d].type &&
            (n[d].default = n[d].default ? n[d].default : a.data.time),
          "time" == n[d].type &&
            (n[d].default = n[d].default ? n[d].default : "00:00"),
          1 == n[d].required)
        )
          if ("radio" == n[d].type || "checkboxc" == n[d].type) {
            var r = !1;
            for (var c in n[d].default_list)
              1 == n[d].default_list[c].is_selected && (r = !0);
            if (!r)
              return (
                wx.showModal({
                  title: "提示",
          ...
          } else if (!n[d].default || void 0 == n[d].default)
            return (
              wx.showModal({
                title: "提示",
                content: "请填写" + o.name + "，加‘*’为必填项",
    ...
    if (
      ((i.form = JSON.stringify(o)),
      a.data.cart_id_list &&
        (i.cart_id_list = JSON.stringify(a.data.cart_id_list)),
      ...
      for (var d in a.data.mch_list)
        if (a.data.mch_list[d].cart_id_list) {
          ...
          a.data.mch_list[d].content &&
            (_.content = a.data.mch_list[d].content),
            l.push(_);
        }
      l.length ? (i.mch_list = JSON.stringify(l)) : (i.mch_list = "");
    }
    a.data.goods_info && (i.goods_info = JSON.stringify(a.data.goods_info)),
      a.data.picker_coupon &&
        (i.user_coupon_id = a.data.picker_coupon.user_coupon_id),
      a.data.content && (i.content = a.data.content),
      a.data.cart_list && (i.cart_list = JSON.stringify(a.data.cart_list)),
      1 == a.data.integral_radio ? (i.use_integral = 1) : (i.use_integral = 2),
      (a.data.goods_list && a.data.goods_list.length) ||
        !a.data.mch_list ||
        1 != a.data.mch_list.length ||
        (i.content = a.data.mch_list[0].content
          ? a.data.mch_list[0].content
          : ""),
      (i.payment = a.data.payment),
      (i.formId = t.detail.formId),
      (i.other_mobile = a.data.other_mobile),
      a.order_submit(i, "s");
  },
  onReady: function() {},
  onShow: function(t) {
    if (
      ...
      1034 != getApp().onShowData.scene
    )
      if (o) o = !1;
      else {
        var e = getCurrentPages();
        console.log(e), a.pageOnShow(this);
        var i = this,
          s = wx.getStorageSync("picker_address");
        if (s) {
          var n = i.data.is_area_city_id,
            d = {};
          n &&
            (!(function(t, a) {
              for (var e = 0; e < t.length; e++) if (a == t[e]) return !1;
  ...
  getOrderData: function(s) {
    var o = this,
      ...
      d = "";
    if (
      (o.data.address && o.data.address.id && (d = o.data.address.id),
      (n.address_id = d),
      (n.longitude = e),
      (n.latitude = i),
      wx.showLoading({
        ...
        success: function(t) {
          if ((wx.hideLoading(), 0 == t.code)) {
            ...
            var n = t.data.shop_list,
              d = {};
            n && 1 == n.length && (d = n[0]),
              t.data.is_shop && (d = t.data.is_shop),
              a ||
                ((a = {
                  shop: d,
                  ...
                  form: t.data.form
                }).pay_type_list.length > 1
                  ? (a.payment = -1)
                  : (a.payment = a.pay_type_list[0].payment)),
              ...
              (a.is_payment = t.data.is_payment),
              (a.mch_list = t.data.mch_list || null),
              (a.is_area_city_id = t.data.is_area_city_id),
              (a.pay_type_list = t.data.pay_type_list),
              (a.offer_rule = t.data.offer_rule),
              o.setData(a),
              o.getInputData(),
              t.data.goods_info &&
                o.setData({
  ...
  pickCoupon: function(t) {
    ...
    var index = isGet.indexOf(id);
    if (index != -1) {
      isGet.splice(index, 1);
    } else {
      isGet = [id];
    }
    // if (parseInt(this.data.isGet) == parseInt(t.currentTarget.dataset.id)) {
    //   this.setData({
    //     isGet: -1
    //   });
    // } else {
    //   this.setData({
    //     isGet: parseInt(t.currentTarget.dataset.id)
    //   });
    // }
    this.setData({
      isGet
    ...
    "-1" == e || -1 == e
      ? ((i.picker_coupon = !1), (i.show_coupon_picker = !1))
      : ((i.picker_coupon = a.data.coupon_list[e]),
        (i.show_coupon_picker = !1)),
      a.setData(i),
      a.getPrice();
  },
  ...
  pickShop: function(t) {
    ...
    wx.removeStorageSync("input_data"),
      "-1" == e || -1 == e
        ? ((i.shop = !1), (i.show_shop = !1))
        : ((i.shop = a.data.shop_list[e]), (i.show_shop = !1)),
      a.setData(i),
      a.getPrice();
  },
  ...
  getPrice: function() {
    ...
    if (
      (t.data.goods_list &&
        t.data.goods_list.length > 0 &&
        (s && 1 == o && (a -= parseFloat(s.forehead)),
        n && (a = (a * n.discount) / 10),
        a <= 0.01 && (a = 0.01),
        0 == d && (a += e)),
      t.data.mch_list && t.data.mch_list.length)
    );
    t.data.isGet.forEach(item => {
      a -= t.data.coupon_list[item].sub_price;
    });
    //   i && (a -= i.sub_price);
    for (var r in t.data.mch_list)
      a += t.data.mch_list[r].total_price + t.data.mch_list[r].express_price;
      a<0?a = 0:a;
    t.setData({
      new_total_price: parseFloat(a.toFixed(2))
    });
    console.log(t.data.new_total_price);
  },
  cardDel: function() {
    this.setData({
      show_card: !1
  ...
  cardTo: function() {
    this.setData({
      ...
      wx.redirectTo({
        url: "/subpages/my-evaluate/my-evaluate"
      });
  },
  ...
  selectForm: function(t) {
    ...
    "checkbox" == o[e].type &&
      (1 == (n = o[e].default_list)[i].is_selected
        ? (n[i].is_selected = 0)
        : (n[i].is_selected = 1),
      (o[e].default_list = n)),
      (s.list = o),
