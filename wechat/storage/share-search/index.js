var t = require("../../api.js"), a = getApp();

Page({
    data: {
        load_more_count: 0,
        last_load_more_time: 0,
        is_loading: !1,
        loading_class: "",
        keyword: !1,
        page: 1,
        limit: 20,
        goods_list: [],
        show_result: !1,
        is_search: !0
    },
    onLoad: function(t) {
        a.pageOnLoad(this);
    },
    onReady: function() {},
    onShow: function() {
        a.pageOnShow(this);
    },
    inputFocus: function() {
        this.setData({
            show_result: !1
        });
    },
    inputBlur: function() {
        var t = this;
        t.data.goods_list.length > 0 && setTimeout(function() {
            t.setData({
                show_result: !0
            });
        }, 300);
    },
    inputConfirm: function(t) {
        var a = this, s = t.detail.value;
        0 != s.length && (a.setData({
            page: 1,
            keyword: s
        }), a.getGoodsList());
    },
    selectShare: function(e) {
        var userId = e.currentTarget.dataset.userId;
        var userName = e.currentTarget.dataset.userName;
        a.request({
            url: t.share.set_parent,
            method: "post",
            data: {
                parent_id: userId,
            },
            success: function(t) {
                0 == t.code && console.log(t);
            },
            complete: function() {
                wx.navigateBack({user_id: userId, user_name: userName});
            }
        });
        console.log(userId, userName, e);
    },
    searchCancel: function() {
        wx.navigateBack({
            delta: 1
        });
    },
    getGoodsList: function() {
        var s = this;
        s.setData({
            show_result: !0,
            is_search: !0
        }), s.setData({
            page: 1,
            scroll_top: 0
        }), s.setData({
            goods_list: []
        });
        var e = {};
        s.data.keyword && (e.keyword = s.data.keyword),
            s.showLoadingBar(), s.is_loading = !0, a.request({
            url: t.share.search,
            data: e,
            success: function(t) {
                0 == t.code && (s.setData({
                    goods_list: t.data.list
                }), 0 == t.data.list.length ? s.setData({
                    is_search: !1
                }) : s.setData({
                    is_search: !0
                })), t.code;
            },
            complete: function() {
                s.hideLoadingBar(), s.is_loading = !1;
            }
        });
    },
    onListScrollBottom: function(t) {
        this.getMoreGoodsList();
    },
    getMoreGoodsList: function() {
        var s = this, e = {};
         s.data.keyword && (e.keyword = s.data.keyword),
            e.page = s.data.page || 1, s.showLoadingMoreBar(), s.setData({
            is_loading: !0
        }), s.setData({
            load_more_count: s.data.load_more_count + 1
        }), e.page = s.data.page + 1, s.setData({
            page: e.page
        }), a.request({
            url: t.share.search,
            data: e,
            success: function(t) {
                if (0 == t.code) {
                    var a = s.data.goods_list;
                    if (t.data.list.length > 0) {
                        for (var i in t.data.list) a.push(t.data.list[i]);
                        s.setData({
                            goods_list: a
                        });
                    } else s.setData({
                        page: e.page - 1
                    });
                }
                t.code;
            },
            complete: function() {
                s.setData({
                    is_loading: !1
                }), s.hideLoadingMoreBar();
            }
        });
    },
    showLoadingBar: function() {
        this.setData({
            loading_class: "active"
        });
    },
    hideLoadingBar: function() {
        this.setData({
            loading_class: ""
        });
    },
    showLoadingMoreBar: function() {
        this.setData({
            loading_more_active: "active"
        });
    },
    hideLoadingMoreBar: function() {
        this.setData({
            loading_more_active: ""
        });
    }
});
